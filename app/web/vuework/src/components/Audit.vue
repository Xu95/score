        ...
        <el-container style="height: 100%; border: 1px solid #eee">
            ...
            <el-col :span="21">
                <el-table :data="tasklist" :default-sort="{prop: 'taskID', order: 'descending'}">
                    ...
                    <el-table-column prop="detail.taskname" label="任务名称" width="400%" sortable>
                    </el-table-column>
                    <el-table-column prop="detail.applicant_name" label="申请人" width="200%"  sortable>
                    </el-table-column>
                    <el-table-column prop="detail.time" label="任务启动时间" width="200%" sortable>
                    </el-table-column>
  ...
  export default {
    ...
    created: function () {
      this.$axios({
        ...
        for (let i in tasklist) {
          this.tasklist.push({id: i, detail: tasklist[i]});
        }
      }, (err) => {
        console.log(err);
      })
    ...
    methods: {
      ...
      getUserName(row, column) {
        //console.log(row.detail.userid);
        return row.userid === 1 ? '男' : row.userid === 0 ? '女' : '未知'
      }
    }
  };
</script>